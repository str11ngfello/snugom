#[tokio::test]
async fn builder_update_disconnects_follower() {
    ...
    let user = users
        .create(
            ...
            UserRecord::validation_builder()
                .display_name("Connect Base".to_string())
    ...
    let connect_patch = UserRecord::patch_builder()
        .entity_id(user_id.clone())
        .expected_version(version)
        .connect("followers_ids", vec![follower_id.clone()]);
    let connect_responses = users
        .update_patch(&mut executor, connect_patch)
        .await
    ...
    let mut executor = RedisExecutor::new(&mut conn);
    let disconnect_patch = UserRecord::patch_builder()
        .entity_id(user_id.clone())
        .expected_version(version)
        .disconnect("followers_ids", vec![follower_id.clone()]);
    users
        .update_patch(&mut executor, disconnect_patch)
        .await
...
#[tokio::test]
async fn builder_delete_detaches_followers() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
...
#[tokio::test]
async fn builder_delete_follower_detaches_from_leaders() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let connect_patch = UserRecord::patch_builder()
        .entity_id(leader_id.clone())
        .expected_version(leader.responses[0]["version"].as_u64().expect("version"))
        .connect("followers_ids", vec![follower_id.clone()]);
    users
        .update_patch(&mut executor, connect_patch)
        .await
...
#[tokio::test]
async fn builder_update_clears_optional_datetime() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let scheduled_at = created_at + Duration::minutes(5);
    let post = posts
        .create(
            &mut executor,
...
#[tokio::test]
async fn builder_mutate_relations_missing_left_errors() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    let mut executor = RedisExecutor::new(&mut conn);
    ...
    let err = users
        .mutate_relations(
            &mut executor,
            vec![RelationPlan::new("followers_ids", vec!["ghost".to_string()], Vec::new())],
        )
        .await
        .expect_err("expected relation validation error");
    drop(executor);
    assert!(matches!(err, RepoError::Validation(_)));
}
#[tokio::test]
async fn builder_update_unknown_relation_errors() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let user = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Alias Tester".to_string())
                .created_at(created_at),
        )
        .await
        .expect("create user");
...
#[tokio::test]
async fn builder_update_returns_validation_error() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
...
#[tokio::test]
async fn builder_update_version_conflict() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let err = users
        .update_patch(
            &mut executor,
            snugom::snug! {
                UserRecord(entity_id = user_id.clone(), expected_version = 99) {
                    display_name: "Should Fail".to_string(),
                }
            },
        )
        .await
...
#[tokio::test]
async fn builder_update_idempotency_replay() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let patch_builder = || {
        snugom::snug! {
            UserRecord(entity_id = user_id.clone(), expected_version = base_version, idempotency_key = "update-1".to_string()) {
                display_name: "Updated Once".to_string(),
            }
        }
    };

    let first = users
        .update_patch(&mut executor, patch_builder())
        .await
        .expect("first update");
    let version_after_first = first[0]["version"].as_u64().expect("version after first");

    let second = users
        .update_patch(&mut executor, patch_builder())
        .await
        .expect("second update");
    let version_after_second = second[0]["version"].as_u64().expect("version after second");
    ...
    assert_eq!(arr[0]["display_name"], Value::String(String::from("Updated Once")));
}
#[tokio::test]
async fn builder_update_mixed_relation_ops() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let follower_a = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Follower A".to_string())
                .created_at(created_at + Duration::seconds(1)),
        )
        .await
    ...
    let follower_b = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Follower B".to_string())
    ...
    let responses = users
        .update_patch(
            ...
            snugom::snug! {
                UserRecord(entity_id = user_id.clone(), expected_version = version) {
                    display_name?: Some("Mixer".to_string()),
    ...
    version = responses[0]["version"].as_u64().expect("version after first");
...
#[tokio::test]
async fn builder_update_nested_post_with_comments() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    let posts: Repo<PostRecord> = ns.post_repo();
    ...
    let user = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Thread Owner".to_string())
                .created_at(created_at),
        )
        .await
        .expect("create user");
   ...
   let post_id = post_ids[0].clone();
    ...
    assert_eq!(comment_arr[0]["body"], Value::String(String::from("First comment")));
}
#[tokio::test]
async fn builder_update_user_profile() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    users
        .update_patch(&mut executor, update_patch)
        .await
        .expect("update user");
    ...
    assert_eq!(arr[0]["display_name"], Value::String(String::from("Updated")));
...
#[tokio::test]
async fn builder_update_without_expected_version() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let created = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Blind Write".to_string())
                .created_at(created_at),
        )
        .await
        .expect("create user");
        ...
        .update_patch(
            ...
            snugom::snug! {
                UserRecord(entity_id = user_id.clone()) {
                    display_name: "Blind Write v2".to_string(),
                }
            },
        )
        .await
        .expect("update without expected version");
    ...
    let second = users
        .update_patch(
            ...
            snugom::snug! {
                UserRecord(entity_id = user_id.clone()) {
                    display_name: "Blind Write v3".to_string(),
                }
            },
        )
        .await
        .expect("second blind update");
    ...
    let json: Value = serde_json::from_str(&json_str).expect("parse json");
    let arr = json.as_array().expect("arr");
    assert_eq!(arr[0]["display_name"], Value::String(String::from("Blind Write v3")));
    assert_eq!(arr[0]["metadata"]["version"], Value::Number(3.into()));
}
#[tokio::test]
async fn builder_update_detects_version_conflict() {
    let mut conn = redis_conn().await;
    let ns = TestNamespace::unique();
    let users: Repo<UserRecord> = ns.user_repo();
    ...
    let created = users
        .create(
            &mut executor,
            UserRecord::validation_builder()
                .display_name("Conflict Source".to_string())
                .created_at(created_at),
        )
        .await
        .expect("create user");
    ...
    let first = users
        .update_patch(
            ...
            snugom::snug! {
                UserRecord(entity_id = user_id.clone(), expected_version = initial_version) {
                    display_name: "Conflict Updated".to_string(),
                }
            },
        )
        .await
        .expect("update with correct version");
    let committed_version = first[0]["version"].as_u64().unwrap();
    ...
    let err = users
        .update_patch(
            ...
            snugom::snug! {
                UserRecord(entity_id = user_id.clone(), expected_version = initial_version) {
                    display_name: "Conflict Second".to_string(),
                }
            },
        )
        .await
